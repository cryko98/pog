...
console.log('--- the rules, offline ---');
{
  ...
  const ins = [];
  for (let t = 200; t < 180_000; t += 520) ins.push({ t: s0 + t, type: 'throw' });
  for (let t = 500; t < 180_000; t += 750) ins.push({ t: s0 + t, type: 'jump' });
  ...
  const late = simulateRun([{ t: s0 + 7_600, type: 'leave', seq: 1 }], seed, s0, s0 + 7_900);
